...
export class GlobalService {
  ...
  getDonatorsAll(): Observable<Array<DonatorPerson>> {
    ...
    return this.http.get<Array<DonatorPerson>>(
      environment.api.donatorsListAll,
      httpOptions
    );
  }
  ...
  logoutWithMessage() {
    this.logout();
    this.handleSuccess("Sua sessão foi finalizada.");
  }
    ...
    switch (error.status) {
      ...
      case 500:
        this.handleErrorMessage(
          `Ocorreu um erro interno. Contate-nos para reportar o erro.`
        );
        break;
      case 400:
